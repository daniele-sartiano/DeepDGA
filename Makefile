...
ed_sub_$(EMBEDDING_DIM)b$(BATCH_SIZE)lstm$(LSTM_LAYER_SIZE)dout$(DROPOUT)ep$(EPOCHS)$(BIDIRECTIONAL)$(NODENSE)$(SPLIT).model: $(TRAIN_TASK_1)
...
botnet_sub_ed$(EMBEDDING_DIM)b$(BATCH_SIZE)lstm$(LSTM_LAYER_SIZE)dout$(DROPOUT)ep$(EPOCHS)$(BIDIRECTIONAL)$(NODENSE)$(SPLIT).model: $(TRAIN_TASK_2)
	export CUDA_VISIBLE_DEVICES="$(GPU)"; python classifier.py train -ed $(EMBEDDING_DIM) -b $(BATCH_SIZE) -e $(EPOCHS) -lstm $(LSTM_LAYER_SIZE) -d $(DROPOUT) $(BIDIRECTIONAL) $(NODENSE) -nb 21 -f $@ < $<
